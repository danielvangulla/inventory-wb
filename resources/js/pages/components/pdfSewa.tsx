...
import { PdfSewaProps } from './pdfModel';
import { formatTgl } from './functions';
import { formatUang } from './helpers';


export function BodyFirst({ doc, invoice }: PdfSewaProps): jsPDF {
    ...
    doc.text(`: ${invoice?.tenant_book?.perusahaan || ''} (${invoice?.tenant_book?.nama_toko})`, xStart2, yStart);
    ...
    doc.text(`: Lt.${invoice?.tenant_book?.tenant?.floor || ''} #${invoice?.tenant_book?.tenant?.no || ''}`, xStart4, yStart + 25);

    return doc;
}

export function BodySecond({ doc, invoice }: PdfSewaProps): jsPDF {
    ...
    if (invoice?.omset && invoice.omset > 0) doc.text(`Revenue`, xStart3 - 60, yStart3);
    if (invoice?.omset && invoice.omset > 0) doc.text(`Share`, xStart3 - 35, yStart3);
    ...
    if (invoice?.is_dp === 2) {
        doc.text(`Deposit Sewa`, xStart2, yStart5a);
        doc.text(`Deposit Service`, xStart2, yStart5a += 5);
        doc.text(`Deposit Telepon`, xStart2, yStart5a += 5);
        // add line for total deposit
        doc.setLineWidth(0.1);
        doc.line(xStart - 5, yStart5a + 2, doc.internal.pageSize.getWidth() - 15, yStart5a + 2);

        doc.setFont('helvetica', 'bold');
        doc.text(`Total Deposit`, xStart2, yStart5a += 7);
        doc.setFont('helvetica', 'normal');

        yStart5a += 2;
        if (invoice?.deposit_count && invoice?.deposit_count > 1) {
            doc.setFont('helvetica', 'italic');
            doc.text(`(Deposit akan ditagihkan ${invoice?.deposit_count} kali)`, xStart2, yStart5a += 5);
            doc.setFont('helvetica', 'normal');
        }
    ...
    if ((invoice?.is_dp === 2 && invoice?.deposit_count && invoice?.deposit_count > 1) || invoice?.is_dp !== 2) {
        if (invoice?.omset && invoice.omset > 0) doc.text(formatUang(invoice?.omset), xStart3 - 60, yStart5a);
        if (invoice?.omset && invoice.omset > 0) doc.text(`${formatUang(invoice?.share_persen || 0)}%`, xStart3 - 35, yStart5a);
        doc.text(`${invoice?.keterangan || '-'}`, xStart2, yStart5a);
    }

    if (invoice?.diskon_jumlah && invoice.diskon_jumlah > 0)
        doc.text(`${invoice?.diskon_ket || '-'}`, xStart2, yStart5a += 5);

    if (invoice?.biaya_jumlah && invoice.biaya_jumlah > 0)
        doc.text(`${invoice?.biaya_ket || '-'}`, xStart2, yStart5a += 5);

    if (invoice?.ppn_jumlah && invoice.ppn_jumlah > 0)
        doc.text(`PPN`, xStart2, yStart5a += 5);

    doc.text(`Materai`, xStart2, yStart5a += 5);


    let yStart5b = yStart5;

    if (invoice?.is_dp === 2) {
        doc.text(`${formatUang(invoice?.deposit_sewa || 0)}`, xStart3 + 15, yStart5b, { align: 'right' });
        doc.text(`${formatUang(invoice?.deposit_service || 0)}`, xStart3 + 15, yStart5b += 5, { align: 'right' });
        doc.text(`${formatUang(invoice?.deposit_telp || 0)}`, xStart3 + 15, yStart5b += 5, { align: 'right' });

        doc.setFont('helvetica', 'bold');
        doc.text(`${formatUang(invoice?.total_deposit || 0)}`, xStart3 + 15, yStart5b += 7, { align: 'right' });
        doc.setFont('helvetica', 'normal');
    ...
    if ((invoice?.is_dp === 2 && invoice?.deposit_count && invoice?.deposit_count > 1) || invoice?.is_dp !== 2){
        doc.text(`Rp.`, xStart3 - 20, yStart5b);
        doc.text(`${formatUang(invoice?.jumlah || 0)}`, xStart3 + 15, yStart5b, { align: 'right' });
    }

    if (invoice?.diskon_jumlah && invoice.diskon_jumlah > 0)
        doc.text(`(${formatUang(invoice?.diskon_jumlah || 0)})`, xStart3 + 15, yStart5b += 5, { align: 'right' });

    if (invoice?.biaya_jumlah && invoice.biaya_jumlah > 0)
        doc.text(`${formatUang(invoice?.biaya_jumlah || 0)}`, xStart3 + 15, yStart5b += 5, { align: 'right' });

    if (invoice?.ppn_jumlah && invoice.ppn_jumlah > 0)
        doc.text(`${formatUang(invoice?.ppn_jumlah || 0)}`, xStart3 + 15, yStart5b += 5, { align: 'right' });

    doc.text(`${formatUang(invoice?.materai || 0)}`, xStart3 + 15, yStart5b += 5, { align: 'right' });
...
export function BodyRekening({ doc, rekening }: PdfSewaProps): jsPDF {
...
export function BodySignature({ doc, invoice, pdfHeader, signed }: PdfSewaProps): jsPDF {
    ...
    const qrCodeString = `${siteDomain}/inv-sewa/${invoice?.uuid}`;
...
export function Footer({ doc, notes }: PdfSewaProps): jsPDF {
