...
const Create: React.FC = () => {
    const breadcrumbs = [
        {
            title: "Tambah Outlet",
            href: "/inventory/outlets/create",
        },
    ];

    const [formData, setFormData] = useState<Outlet>(initialFormData);
    ...
    const handleSubmit = async () => {
        if (formData.nama.trim() === "") {
            setError("Silahkan isi nama outlet..");
            return;
        }
        ...
        try {
            ...
            router.post(route("inventory.outlets.store"), payload, {
                onSuccess: () => {
                    setSuccess(true);
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="container mx-auto max-w-xl p-2">
                <div className="mb-6 flex flex-row items-center pt-2">
                    <h1 className="text-xl font-bold">Tambah Outlet</h1>
                </div>

                <div className="space-y-4 bg-gray-200 p-4 rounded shadow">

                    <div className="flex flex-col justify-center gap-4 py-4 px-2 bg-gray-100 dark:bg-gray-700 rounded-lg border-2 border-black dark:border-gray-600">
                        {/* Nama Outlet */}
                        <div className="flex flex-col">
                            <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 pl-1">Nama Outlet</label>
                            <input
                                type="text"
                                value={formData.nama}
                    ...
                    {success && <div className="text-green-600">Outlet berhasil dibuat!</div>}
