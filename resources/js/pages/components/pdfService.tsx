...
import { PdfServiceProps } from './pdfModel';
import { formatTgl } from './functions';
import { formatUang } from './helpers';


export function BodyFirst({ doc, invoice }: PdfServiceProps): jsPDF {
    ...
    doc.text(`: Lt.${invoice?.floor || ''} #${invoice?.unit || ''}`, xStart4, yStart + 25);

    return doc;
}

export function BodySecond({ doc, invoice }: PdfServiceProps): jsPDF {
    ...
    let yStart5a = yStart5; // Reset yStart5 for content
    doc.text(`${invoice?.keterangan || '-'}`, xStart2, yStart5a);

    doc.text(`Promotion Levy ${formatUang(invoice?.promotion_levy_persen || 0)}%`, xStart2, yStart5a += 5);

    if (invoice?.diskon_jumlah && invoice?.diskon_jumlah > 0) {
        yStart5a += 5; // Move down for the next line
        doc.text(`${invoice?.diskon_ket || '-'}`, xStart2, yStart5a);
    }

    if (invoice?.biaya_jumlah && invoice?.biaya_jumlah > 0) {
        yStart5a += 5; // Move down for the next line
        doc.text(`${invoice?.biaya_ket || '-'}`, xStart2, yStart5a);
    }
    ...
    let yStart5b = yStart5; // Reset yStart5b for amounts
    doc.text(`Rp.`, xStart3 - 30, yStart5b);
    doc.text(`${formatUang(invoice?.jumlah || 0)}`, xStart3 + 15, yStart5b, { align: 'right' });

    doc.text(`${formatUang(invoice?.promotion_levy_jumlah || 0)}`, xStart3 + 15, yStart5b += 5, { align: 'right' });

    if (invoice?.diskon_jumlah && invoice?.diskon_jumlah > 0) {
        yStart5b += 5; // Move down for the next line
        doc.text(`(${formatUang(invoice?.diskon_jumlah || 0)})`, xStart3 + 15, yStart5b, { align: 'right' });
    }

    if (invoice?.biaya_jumlah && invoice?.biaya_jumlah > 0) {
        yStart5b += 5; // Move down for the next line
        doc.text(`${formatUang(invoice?.biaya_jumlah || 0)}`, xStart3 + 15, yStart5b, { align: 'right' });
    }
    ...
    let yStart5c = yStart5a + 3; // Reset yStart5c for the line below amounts
    doc.setLineWidth(0.1);
    ...
    yStart5c += 5; // Move down for the total line
    ...
    yStart5c += 3; // Move down for the total line
    doc.setLineWidth(0.1);
    doc.line(xStart - 5, yStart5c, doc.internal.pageSize.getWidth() - 15, yStart5c);
    ...
    const totalYWithLines = yStart5c + 5; // Start position for terbilang text
    lines.forEach((line: string | string[], index: number) => {
...
export function BodyRekening({ doc, rekening }: PdfServiceProps): jsPDF {
...
export function BodySignature({ doc, invoice, pdfHeader, signed }: PdfServiceProps): jsPDF {
    ...
    const qrCodeString = `${siteDomain}/inv-serv/${invoice?.uuid}`;
...
export function Footer({ doc, notes }: PdfServiceProps): jsPDF {
