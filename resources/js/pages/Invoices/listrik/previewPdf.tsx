...
const PreviewPdf: React.FC<PreviewPdfProps> = ({ isGuest, invoice, downloadPDF, closeModal }) => {
    return (
        <div className="bg-white p-2 rounded shadow-md w-full md:w-2/3 max-w-2xl mx-2">
            <div className='flex justify-center items-center bg-blue-100 text-black py-2 rounded-t text-xs'>
                ...
                <table className="mx-1">
                    <tbody>
                        ...
                        <tr >
                            <td className="text-left pl-4" colSpan={3}>{invoice.meter_listrik?.tenant_book?.nama_toko}</td>
                        ...
                        <tr >
                            <td className="text-left pl-4 max-w-3xs" colSpan={3}>{invoice.meter_listrik?.tenant_book?.perusahaan}</td>
                        ...
                        <tr >
                            <td className="text-left pl-4 max-w-3xs" colSpan={3}>{invoice.meter_listrik?.tenant_book?.alamat}</td>
                        </tr>
                        <tr>
                        ...
                        <tr >
                            <td className="text-left pl-4 max-w-3xs" colSpan={3}>{`{Nama Owner}`}</td>
                            <td className="text-left pl-4 whitespace-nowrap"><b>Unit No.</b></td>
                            <td className="text-center" width={15}>:</td>
                            <td className="text-left whitespace-nowrap">
                                Lt.{invoice.meter_listrik?.tenant_book?.tenant?.floor} #{invoice.meter_listrik?.tenant_book?.tenant?.no}
                            </td>
                        </tr>
                    </tbody>
                </table>
                ...
                <table className="min-w-full border-collapse">
                    <thead className="border-y border-l border-r border-gray-300 text-md">
                        <tr>
                            ...
                            <th className="px-2 py-1 text-center">Awal <br />kWh</th>
                            <th className="px-2 py-1 text-center">Akhir <br />kWh</th>
                            <th className="px-2 py-1 text-center">Pemakaian <br />kWh</th>
                            <th className="px-2 py-1 text-center">Tarif <br />per kWh</th>
                            <th className="px-2 py-1 text-right">Jumlah</th>
                        </tr>
                    </thead>
                    <tbody className="border-y border-l border-r border-gray-300">
                        <tr className='hover:bg-gray-100'>
                            <td className="px-2 pt-1 text-left">{invoice.keterangan}</td>
                            <td className="px-2 text-center">{formatDigit(invoice.meter_listrik?.awal || 0, 2)}</td>
                            <td className="px-2 text-center">{formatDigit(invoice.meter_listrik?.akhir || 0, 2)}</td>
                            <td className="px-2 text-center">{formatDigit(invoice.pemakaian || 0, 2)}</td>
                            <td className="px-2 text-center">{formatDigit(invoice.tarif, 2)}</td>
                            <td className="px-2 text-right">{formatUang(invoice.subtotal)}</td>
                        </tr>

                        <tr className='hover:bg-gray-100'>
                            <td className="px-2 text-left" colSpan={5}>PPJ ({formatUang(invoice.ppj_persen)}%)</td>
                            <td className="px-2 text-right">{formatUang(invoice.ppj_jumlah)}</td>
                        </tr>

                        <tr className='hover:bg-gray-100'>
                            <td className="px-2 text-left" colSpan={5}>Backup Genset & Loss Energy ({formatUang(invoice.genset_persen)}%)</td>
                            <td className="px-2 text-right">{formatUang(invoice.genset_jumlah)}</td>
                        </tr>

                        <tr className='hover:bg-gray-100'>
                            <td className="px-2 text-left" colSpan={5}>Biaya Admin</td>
                            <td className="px-2 text-right">{formatUang(invoice.biaya_admin)}</td>
                        </tr>

                        {invoice.biaya_lain > 0 &&
                            <tr className='hover:bg-gray-100'>
                                <td className="px-2 text-left" colSpan={5}>Biaya Lain</td>
                                <td className="px-2 text-right">{formatUang(invoice.biaya_lain)}</td>
                            </tr>}

                        {invoice.denda > 0 &&
                            <tr className='hover:bg-gray-100'>
                                <td className="px-2 text-left" colSpan={5}>Denda</td>
                                <td className="px-2 text-right">{formatUang(invoice.denda)}</td>
                            </tr>}

                        <tr className='hover:bg-gray-100'>
                            <td className="px-2 text-left" colSpan={5}>PPN</td>
                            <td className="px-2 text-right">{formatUang(invoice.ppn_jumlah)}</td>
                        </tr>

                        <tr className='hover:bg-gray-100'>
                            <td className="px-2 pb-1 text-left" colSpan={5}>Materai</td>
                    ...
                    <tfoot className="border-y border-l border-r border-gray-300">
                        <tr className='hover:bg-gray-100 font-bold text-md'>
                            <td className="px-2 py-1 text-left" colSpan={4}>Total</td>
                            <td className="px-2 py-1 text-right">Rp.</td>
                            <td className="px-2 py-1 text-right">{formatUang(invoice.tagihan)}</td>
                        </tr>
                    </tfoot>
                </table>
                <br />
