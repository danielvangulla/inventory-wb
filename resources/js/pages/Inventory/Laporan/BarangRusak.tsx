...
const LaporanBarangRusak: React.FC<Props> = ({ data, barangs, suppliers, kategoris, params }) => {
    ...
    const handleFilterByTanggal = (tgl1: string, tgl2: string) => {
        ...
        router.visit(`/inventory/laporan-barang-rusak`, {
            method: 'post',
            data: {
                tgl1: tgl1,
                tgl2: tgl2,
    ...
    const [filteredData, setFilteredData] = useState<BarangRusakDetail[]>(data);
    ...
    const filterBySearch = (currentData: BarangRusakDetail[], query: string) => {
        return currentData.filter(item =>
            item.barang_rusak?.supplier?.nama.toLowerCase().includes(query.toLowerCase()) ||
    ...
    const handlePrint = () => {
        const printUrl = `/inventory/laporan-barang-rusak-print/${tglMulai}/${tglSelesai}/${selectedBarangOption?.value}/${selectedSupplierOption?.value}/${selectedKategoriOption?.value}/${selectedKategorisubOption?.value}`;
