...
// pages/Invoices/service/index.tsx
...
const Index: React.FC<Props> = ({ username, rekening, tenantBooks, invoices, signs, cities }) => {
    ...
    const handleModalOpen = () => {
        ...
        setTimeout(async () => {
            try {
                const respNumbers = await fetch('/invoice-service-list');
    ...
    const handleSubmit = (invoice: any) => {
        ...
        router.post('/invoices-service', invoice, {
            onSuccess: (page) => {
                setSuccessMessage(
    ...
    const cancelInvoice = (invoice: ServiceInvoice) => {
        if (loading) return;
        ...
        router.delete(`/invoices-service/${invoice.id}`, {
            onSuccess: (page) => {
                setSuccessMessage(
                    typeof page.props.success === 'string'
                        ? page.props.success
    ...
    const handlePrintKuitansi = (invoice: ServiceInvoice) => {
        if (loading) return;
        setLoading(true);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="INVOICES - SERVICE CHARGE" />
                        ...
                        <table className="table-auto w-full ">
                            <thead>
                                <tr>
                                    ...
                                    <th className="p-2 border border-black dark:border-gray-400 text-center">Potongan / Biaya</th>
                                    <th className="p-2 border border-black dark:border-gray-400 text-center">Promotion Levy</th>
                            ...
                            <tbody>
                                {invoices.map((v, index) => (
                                    <tr key={index} className='text-sm hover:bg-blue-300 dark:hover:bg-blue-500 transition-colors duration-100'>
                                        ...
                                        <td className="py-1 px-2 border border-black dark:border-gray-400 text-center whitespace-nowrap">
                                            <b>{formatUang(v.biaya_jumlah - v.diskon_jumlah)}</b>
                                        </td>
                                        <td className="py-1 px-2 border border-black dark:border-gray-400 text-center whitespace-nowrap">
                                            <b>{formatUang(v.promotion_levy_jumlah)}</b> <br />
                                            <span className='text-xs italic'>({validateDecimal(v.promotion_levy_persen)}%)</span>
                                        </td>
                                        <td className="py-1 px-2 border border-black dark:border-gray-400 text-center whitespace-nowrap">
