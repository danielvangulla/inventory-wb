...
const Index: React.FC<Props> = ({ username, tenantBooks }) => {
    ...
    const handleSubmit = (meterAir: MeterAir) => {
        if (loading) return;
        setModalOpen(false);
        setLoading(true);
        setErrorMessage('');
        ...
        const data = {
            ...
            tgl: meterAir.tgl,
            awal: meterAir.awal,
            akhir: meterAir.akhir,
            pemakaian: meterAir.pemakaian,
        }

        router.post('/meter-air', data, {
            onSuccess: (page) => {
                setSuccessMessage(
                            ...
                            <tbody>
                                {tenantBooks?.map((tenant, i) => (
                                    <tr key={tenant.id} className="hover:bg-blue-300 dark:hover:bg-blue-500 transition-colors duration-100">
                                        ...
                                        <td className="px-2 py-0.5 border border-black dark:border-gray-400 text-center">
                                            {tenant.meter_air_last ? monthYear(new Date(tenant.meter_air_last.tgl)) : '-'}
                                        </td>
                                        <td className="px-2 py-0.5 border border-black dark:border-gray-400 text-center">
                                            {tenant.meter_air_last ? `${validateDecimal(tenant.meter_air_last.akhir)} m³` : '-'}
                                        </td>
                                        <td className="px-2 py-0.5 border border-black dark:border-gray-400 text-center">
                                            {tenant?.meter_air_now ? monthYear(new Date(tenant.meter_air_now.tgl)) : '-'}
                                        </td>
                                        <td className="px-2 py-0.5 border border-black dark:border-gray-400 text-center">
                                            {tenant.meter_air_now ? `${validateDecimal(tenant.meter_air_now.akhir)} m³` : '-'}
                                        </td>
                                        <td className="px-2 py-0.5 border border-black dark:border-gray-400 text-center">
                                            {tenant.meter_air_now && tenant.meter_air_last
                                                ? `${validateDecimal(tenant.meter_air_now.akhir - tenant.meter_air_last.akhir)} m³`
                                                : '-'}
                                        </td>
                                    </tr>
                                ))}
