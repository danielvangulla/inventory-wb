...
import { PdfAirProps } from './pdfModel';
import { formatTgl } from './functions';
import { formatDigit, formatUang } from './helpers';

export function BodyFirst({ doc, invoice }: PdfAirProps): jsPDF {
    const tenantBook = invoice?.meter_air?.tenant_book;
...
export function BodySecond({ doc, invoice }: PdfAirProps): jsPDF {
    ...
    const yStart4 = yStart3 + 4; // Y position for the header line under the text
    ...
    let yIsi1 = yStart5;
    doc.text(`${invoice?.keterangan || '-'}`, xHead1, yIsi1);
    doc.text(`${formatDigit(invoice?.meter_air?.awal || 0, 2)}`, xHead2, yIsi1, { align: 'center' });
    doc.text(`${formatDigit(invoice?.meter_air?.akhir || 0, 2)}`, xHead3, yIsi1, { align: 'center' });
    doc.text(`${formatDigit(invoice?.pemakaian || 0, 2)}`, xHead4, yIsi1, { align: 'center' });

    invoice?.air_invoice_details?.forEach((detail) => {
        yIsi1 += 6; // Increment Y position for each detail
        doc.text(`${detail?.ket || '-'}`, xHead1, yIsi1);
        doc.text(`${formatDigit(detail?.volume || 0, 2)}`, xHead4, yIsi1, { align: 'center' });
        doc.text(`${formatDigit(detail?.tarif || 0, 2)}`, xHead5, yIsi1, { align: 'center' });
        doc.text(`${formatUang(detail?.jumlah || 0)}`, xHead6, yIsi1, { align: 'right' });
    });

    const yIsi2 = yIsi1 + 6;
    doc.text(`Biaya Admin`, xHead1, yIsi2);
    doc.text(`${formatUang(invoice?.biaya_admin || 0)}`, xHead6, yIsi2, { align: 'right' });

    let yIsi3 = yIsi2;
    if (invoice?.biaya_lain && invoice?.biaya_lain > 0) {
        yIsi3 += 6;
        doc.text(`Biaya Lain`, xHead1, yIsi3);
        doc.text(`${formatUang(invoice?.biaya_lain || 0)}`, xHead6, yIsi3, { align: 'right' });
    }

    let yIsi4 = yIsi3;
    if (invoice?.denda && invoice?.denda > 0) {
        yIsi4 += 6;
        doc.text(`Denda`, xHead1, yIsi4);
        doc.text(`${formatUang(invoice?.denda || 0)}`, xHead6, yIsi4, { align: 'right' });
    }

    yIsi4 += 6;
    doc.text(`PPN`, xHead1, yIsi4);
    doc.text(`${formatUang(invoice?.ppn_jumlah || 0)}`, xHead6, yIsi4, { align: 'right' });

    yIsi4 += 6;
    doc.text(`Materai`, xHead1, yIsi4);
    doc.text(`${formatUang(invoice?.materai || 0)}`, xHead6, yIsi4, { align: 'right' });

    doc.setLineWidth(0.1);
    doc.line(xStart - 5, yIsi4 + 2, doc.internal.pageSize.getWidth() - 15, yIsi4 + 2);

    // Total amount, bold the font
    const yTotal = yIsi4 + 7;
    doc.setFont('helvetica', 'bold');
...
export function BodyRekening({ doc, rekening }: PdfAirProps): jsPDF {
...
export function BodySignature({ doc, invoice, pdfHeader, signed }: PdfAirProps): jsPDF {
    ...
    const qrCodeString = `${siteDomain}/inv-air/${invoice?.uuid}`;
...
export function Footer({ doc, notes }: PdfAirProps): jsPDF {
