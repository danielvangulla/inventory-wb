...
import { Kategorisub } from '../models';
...
const Index: React.FC<Props> = ({ kategorisub }) => {
    const [filteredKategori, setFilteredKategori] = useState<Kategorisub[]>(kategorisub);

    const handleSearch = (query: string) => {
        const filtered = kategorisub.filter(item =>
    ...
    const handleDelete = (id: number) => {
        const ket = kategorisub.find(t => t.id === id)?.ket || 'this kategori';

        if (confirm(`Hapus Kategori ${ket} ?`)) {
            router.delete(route('inventory.kategorisub.destroy', id));
        }
    }
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="w-full px-4 py-2 max-w-lg md:max-w-xl lg:max-w-3xl xl:max-w-5xl 2xl:max-w-full mx-auto">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="mb-2">
                        <Link
                            href={route('inventory.kategorisub.create')}
                    ...
                    <table className="bg-white dark:bg-gray-800 shadow-md rounded-md min-w-full">
                        <thead>
                            <tr className="bg-gray-200 dark:bg-gray-700 border border-gray-300 dark:border-gray-600">
                                ...
                                <th className="py-2 px-2 text-center border border-black dark:border-gray-400">Sub-Kategori</th>
                        ...
                        <tbody>
                            ...
                            {filteredKategori.length > 0 && filteredKategori.map((v, index) => (
                                <tr key={v.id} className='hover:bg-blue-300 dark:hover:bg-gray-600'>
                                    ...
                                    <td className="py-2 px-2 text-left border border-black dark:border-gray-400">{v.kategori?.ket}</td>
                                    ...
                                    <td className="py-2 px-1 text-center border border-black dark:border-gray-400">
                                        ...
                                        <Link
                                            title={`Edit ${v.ket}`}
                                            href={route('inventory.kategorisub.edit', v.id)}
