                ...
                <table className="w-full border-collapse border border-black">
                    <thead>
                        <tr className="bg-gray-300">
                            ...
                            <th className="border border-black p-3">Harga</th>
                    ...
                    <tbody>
                        {data.details && data.details.map((detail, index) => (
                            <tr key={detail.id}>
                                ...
                                <td className="border border-black px-2 py-2 text-center">{formatDigit(detail.harga, 2)}</td>
                                <td className="border border-black px-2 py-2 text-center">{formatDigit(detail.qty, 2)} {detail.barang?.satuan}</td>
                                <td className="border border-black px-2 py-2 text-center">{formatDigit(detail.total, 2)}</td>
                            </tr>
                        ))}
                    </tbody>
                    ...
                    <tfoot>
                        <tr className="text-lg font-bold">
                            <td colSpan={4} className="border border-black p-3 text-right">Total Bayar Rp.</td>
                            <td className="border border-black p-2 text-center">{formatDigit(data.total, 2)}</td>
                        </tr>
                    </tfoot>
                </table>
