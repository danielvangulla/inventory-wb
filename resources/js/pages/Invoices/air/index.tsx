...
const breadcrumbs: BreadcrumbItem[] = [
    {
        title: 'INVOICES - REKENING AIR',
        href: '/invoices-air',
    },
];

const Index: React.FC<Props> = ({ username, tenantBooks, rekening, invoices, signs, tarifs, cities }) => {
    ...
    const handleModalOpen = () => {
        ...
        setTimeout(async () => {
            try {
                const respNumbers = await fetch('/invoice-air-list');
    ...
    const handleSubmit = (invoice: any) => {
        ...
        router.post('/invoices-air', invoice, {
            onSuccess: (page) => {
                setSuccessMessage(
    ...
    const cancelInvoice = (invoice: AirInvoice) => {
        if (loading) return;
        ...
        router.delete(`/invoices-air/${invoice.id}`, {
            onSuccess: (page) => {
                setSuccessMessage(
                    typeof page.props.success === 'string'
                        ? page.props.success
            ...
            onError: (error) => {
                setErrorMessage(error.message || 'An error occurred while canceling the invoice..');
                setIsErrorModalOpen(true);
            },
            onFinish: () => {
                setLoading(false);
    ...
    const handlePrintKuitansi = (invoice: AirInvoice) => {
        if (loading) return;
        setLoading(true);
        ...
        setTimeout(async () => {
            try {
                const arrKuitansi = {
                    ...
                    keterangan,
                    jenis: `air`,
                    tenant_book_id: invoice.meter_air?.tenant_book_id || 0,
                    invoice_id: invoice.id || 0,
                    invoice_no: invoice.no,
                    tenant_book: invoice.meter_air?.tenant_book,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="INVOICES - REKENING AIR" />
                                ...
                                {invoices.map((v) => (
                                    <tr key={v.id} className="hover:bg-blue-300 dark:hover:bg-blue-500 transition-colors duration-100">
                                        <td className="p-2 border border-black dark:border-gray-400 text-left whitespace-nowrap">
                                            <b>{v.meter_air?.tenant_book?.nama_toko}</b> <br />
                                            {v.meter_air?.tenant_book?.perusahaan}
                                        </td>
                                        <td className="p-2 border border-black dark:border-gray-400 text-center whitespace-nowrap">
                                            Lt. {v.meter_air?.tenant_book?.tenant?.floor} <br />
                                            Unit {v.meter_air?.tenant_book?.tenant?.no}
                                        </td>
                                        ...
                                        <td className="p-2 border border-black dark:border-gray-400 text-center">
                                            {formatDigit(v.meter_air?.pemakaian || 0, 2)} m<sup>3</sup>
                                        </td>
                                        ...
                                        <td className="p-2 border border-black dark:border-gray-400 text-right">
                                            ...
                                            (<span className='text-xs italic'>{v.ppn_persen}%</span>)
                                        </td>
                                        <td className="p-2 border border-black dark:border-gray-400 text-right">
                                            {formatUang(v.materai)}
                                        </td>
                                        <td className="p-2 border border-black dark:border-gray-400 text-right">
                                            <b>{formatUang(v.tagihan)}</b>
                                        </td>
                                        ...
                                        <td className="p-2 border border-black dark:border-gray-400 text-center white">
