...
const LaporanPembelian: React.FC<Props> = ({ data, barangs, suppliers, kategoris, params }) => {
    ...
    const handleFilterByTanggal = (tgl1: string, tgl2: string) => {
        ...
        router.visit(`/inventory/laporan-pembelian/${tgl1}/${tgl2}/${selectedBarangOption?.value}/${selectedSupplierOption?.value}/${selectedKategoriOption?.value}/${selectedKategorisubOption?.value}`);
    };
                    ...
                    <table className="bg-white shadow-md rounded-md min-w-full">
                        ...
                        <tfoot>
                            <tr className='bg-blue-200 text-lg font-bold'>
                                <td colSpan={6} className="py-2 px-2 text-right border border-black dark:border-gray-400 font-bold">Total Rp.</td>
